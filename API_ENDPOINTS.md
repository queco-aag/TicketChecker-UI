...
**Endpoint:** `GET /api/numeros/{numero}/verificar`
...
const response = await numerosAPI.verificar('12345');
const data = mapNumeroVerificationResponse(response.data);
```
...
**Endpoint:** `POST /api/numeros/{numero}/reclamar`
...
```javascript
import { numerosAPI } from './services/api';
import { prepareClaimData } from './services/apiMappers';

const formData = new FormData();
formData.append('nombre', 'Juan Pérez');
formData.append('contacto', 'juan@email.com');
formData.append('direccionEnvio', 'Calle Mayor 123');
formData.append('comprobante', file);

const response = await numerosAPI.reclamar('12345', formData);
```
...
**Endpoint:** `POST /api/premios/cargar-csv`
...
**Endpoint:** `PUT /api/premios/{id}/marcar-enviado`
...
await premiosAPI.marcarEnviado(1);
```
...
**Endpoint:** `GET /api/premios/enviados`
...
```javascript
import { premiosAPI } from './services/api';
import { mapNumerosPremiados } from './services/apiMappers';

const response = await premiosAPI.obtenerEnviados();
const premios = mapNumerosPremiados(response.data);
```
...
**Endpoint:** `GET /api/premios/pendientes`
...
**Endpoint:** `GET /api/premios/reclamados`
...
**Endpoint:** `POST /api/auth/first-admin`
...
El interceptor de Axios en `services/api.js` maneja esto automáticamente para endpoints que incluyen `/admin` en la URL.
...
- [OpenAPI Specification](../TicketChecker/src/main/resources/openapi.yaml) - Especificación completa de la API
- [README.md](./README.md) - Documentación principal del proyecto
- [MIGRATION_GUIDE.md](./MIGRATION_GUIDE.md) - Guía de migración v2.0
